...
const Todos = ({ todos }) => {
    const todoList = todos.length ? ( //if the todos array has items in the list [todos.length]
      todos.map((todo) => {  //return the list
        return (
          <Grid key={todo.id}>
            <Card>
    ...
    ) : ( //else, return "you have no todo's left"
      <p>{ "You have no todo's left" /*Enter your message here if todo list is empty*/}</p>
    );
